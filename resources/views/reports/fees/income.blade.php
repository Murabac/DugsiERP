...
<div class="space-y-4">
    ...
    <x-section-header :title="$labels['page_title']" :sub="$labels['sub'].' · AY '.$academicYear">
        <x-slot:action>
            <x-lang-toggle route="reports.fees.income" :lang="$lang" />
            <x-btn variant="secondary" :href="route('reports.fees.income.print', request()->query())" target="_blank">{{ $labels['print'] }}</x-btn>
        </x-slot:action>
    </x-section-header>

    <form method="GET" action="{{ route('reports.fees.income') }}" class="rounded-lg border border-slate-200 bg-white p-4">
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
            <div>
                ...
                <div class="flex items-center gap-1">
                    @if ($prevMonth)
                        <a href="{{ route('reports.fees.income', ['month' => $prevMonth, 'lang' => $lang]) }}"
                    ...
                    @if ($nextMonth)
                        <a href="{{ route('reports.fees.income', ['month' => $nextMonth, 'lang' => $lang]) }}"
                            class="rounded-md border border-slate-200 bg-white px-2.5 py-2 text-sm text-slate-700 hover:bg-slate-50">›</a>
                    @endif
                </div>
            </div>
