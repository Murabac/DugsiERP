...
@section('back_url', route('reports.fees.net-income', request()->except('print')))
...
@endsection
@section('counts')
    {{ $labels['net_total'] }} {{ \App\Support\Money::format($net) }}
@endsection
...
<table class="data" style="border-top:0;border-radius:0;margin-bottom:12px;">
    <thead>
        <tr>
    ...
    <tfoot>
        <tr>
            <td>{{ $labels['income_total'] }}</td>
            <td class="num">{{ \App\Support\Money::format($incomeTotal) }}</td>
        </tr>
    </tfoot>
</table>
...
<table class="data" style="border-top:0;border-radius:0;margin-bottom:12px;">
    ...
    <tfoot>
        <tr>
            <td>{{ $labels['expense_total'] }}</td>
            <td class="num">{{ \App\Support\Money::format($expenseTotal) }}</td>
        </tr>
    </tfoot>
</table>

<div style="background:#334155;color:#fff;text-align:center;padding:8px 10px;font-weight:600;font-size:13px;margin-bottom:0;border-radius:4px 4px 0 0;">
    {{ $labels['section_net'] }}
</div>
<table class="data" style="border-top:0;border-radius:0 0 4px 4px;">
    <tbody>
        <tr>
            <td style="width:70%">{{ $labels['income_total'] }}</td>
            <td style="width:30%" class="num">{{ \App\Support\Money::format($incomeTotal) }}</td>
        </tr>
        <tr>
            <td>{{ $labels['expense_total'] }}</td>
            <td class="num">− {{ \App\Support\Money::format($expenseTotal) }}</td>
    ...
    <tfoot>
        <tr>
            <td>{{ $labels['net_total'] }}</td>
            <td class="num">{{ \App\Support\Money::format($net) }}</td>
        </tr>
    </tfoot>
</table>
