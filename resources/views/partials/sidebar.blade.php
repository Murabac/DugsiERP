...
<aside id="app-sidebar"
    ...
    <div class="flex items-center justify-between border-b border-white/10 px-4 py-4">
        <a href="{{ route('modules.home') }}" class="flex min-w-0 items-center gap-2.5 rounded-md outline-none hover:opacity-90 focus-visible:ring-2 focus-visible:ring-white/40">
            ...
            <div class="min-w-0">
                ...
                <div class="text-[11px] text-blue-300/80">{{ $user->roleLabel() }}</div>
            </div>
        </a>
    ...
    <nav class="flex-1 space-y-0.5 overflow-y-auto px-2 py-3">
        @foreach ($navItems as $item)
            @if (($item['type'] ?? 'item') === 'group')
                @php
                    $groupActive = collect($item['children'])->contains(function ($c) {
                        if (request()->routeIs($c['route'])) {
                            return true;
                        }

                        // Staff attendance: nav points at history, but day view is the same module.
                        return ($c['route'] ?? '') === 'staff-attendance.history'
                            && request()->routeIs('staff-attendance.*');
                    });
                @endphp
                <details class="group/nav" @if ($groupActive || request()->routeIs('finance.*') || request()->routeIs('timetable.*')) open @endif>
                        ...
                        @foreach ($item['children'] as $child)
                            @php
                                $childActive = request()->routeIs($child['route'])
                                    || (($child['route'] ?? '') === 'staff-attendance.history' && request()->routeIs('staff-attendance.*'));
                            @endphp
                            <a href="{{ route($child['route']) }}"
                                class="flex w-full items-center gap-2 rounded-md px-2.5 py-1.5 text-left text-[12px] transition-colors {{ $childActive ? 'bg-blue-600 font-medium text-white' : 'text-blue-100/50 hover:bg-white/8 hover:text-white' }}">
                                <span class="h-1 w-1 flex-shrink-0 rounded-full {{ $childActive ? 'bg-white' : 'bg-white/30' }}"></span>
                                {{ $child['label'] }}
                            </a>
                        @endforeach
                    </div>
            ...
            @else
                @php
                    $isAppsHome = ($item['route'] ?? '') === 'modules.home';
                    $itemActive = ! $isAppsHome && request()->routeIs($item['route']);
                @endphp
                <a href="{{ route($item['route']) }}"
                    class="flex w-full items-center gap-2.5 rounded-md px-3 py-2 text-left text-[13px] transition-colors {{ $isAppsHome ? 'mb-1 border border-white/15 bg-white/5 font-medium text-white hover:bg-white/10' : ($itemActive ? 'bg-blue-600 font-medium text-white' : 'text-blue-100/60 hover:bg-white/8 hover:text-white') }}">
                    <x-icon :name="$item['icon']" :size="15" />
                    {{ $item['label'] }}
                </a>
            @endif
