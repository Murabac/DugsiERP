...
<head>
    <meta charset="utf-8">
    <title>Fee Receipt — {{ $payment->receipt_number }} — {{ $schoolName }}</title>
    <style>
        * { box-sizing: border-box; }
        ...
        .actions a, .actions button { padding: 8px 14px; border-radius: 6px; border: 0; cursor: pointer; font-size: 13px; text-decoration: none; display: inline-block; }
        .btn-primary { background: #1e3a6e; color: #fff; }
        .btn-secondary { background: #fff; color: #1e3a6e; border: 1px solid #cbd5e1 !important; }
        @media print { body { background: #fff; padding: 0; } .no-print { display: none !important; } .sheet { border: 0; max-width: none; padding: 0; } }
    </style>
</head>
<body>
...
<div class="sheet">
    <div class="header">
        ...
        <div>
            ...
            <div class="sub" style="text-align:right;">{{ $payment->receipt_number }}</div>
    ...
    <div class="meta">
        ...
        <div><div class="label">Class</div><div>{{ $invoice->schoolClass?->displayName() ?? '—' }}</div></div>
        <div><div class="label">Paid at</div><div>{{ $payment->paid_at?->format('j F Y, H:i') }}</div></div>
        <div><div class="label">Method</div><div>{{ $payment->method->label() }}</div></div>
        <div><div class="label">Recorded by</div><div>{{ $payment->recordedBy?->name ?? '—' }}</div></div>
    </div>
    ...
    <table>
        <thead><tr><th>Applied to</th><th>Invoice</th><th style="text-align:right;">USD</th></tr></thead>
        <tbody>
